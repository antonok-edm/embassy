...
/// Sdmmc device
pub struct Sdmmc<'d, T: Instance, P: SdmmcPeripheral> {
    _peri: Peri<'d, T>,
    #[cfg(sdmmc_v1)]
...
#[cfg(sdmmc_v1)]
impl<'d, T: Instance, P: SdmmcPeripheral> Sdmmc<'d, T, P> {
...
impl<'d, T: Instance> Sdmmc<'d, T, Card> {
    ...
    async fn get_scr(&mut self, card: &mut Card) -> Result<(), Error> {
        ...
        Self::cmd(common_cmd::app_cmd(card.get_address()), false)?;
...
impl<'d, T: Instance> Sdmmc<'d, T, Emmc> {
...
impl<'d, T: Instance, P: SdmmcPeripheral> Drop for Sdmmc<'d, T, P> {
    fn drop(&mut self) {
        T::Interrupt::disable();
        Self::on_drop();
...
impl<'d, T: Instance, P: SdmmcPeripheral> block_device_driver::BlockDevice<512> for Sdmmc<'d, T, P> {
    type Error = Error;
